<template>
  <div>
    ...
    <div v-else class="table-card">
      ...
      <div v-if="listCustomers.length > 0" class="pagination-controls" style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 15px; padding: 10px; background: #f8f9fa; border-radius: 8px; border: 1px solid #e1e4e8;">
      ...
      <table>
        ...
        <tbody>
          <tr v-for="(kh, index) in paginatedCustomers" :key="kh.ma_khach_hang" 
            ...
            <td style="text-align: center;">
              <div v-if="hasRole(1)" style="display: flex; gap: 8px; justify-content: center;">
                <button class="action-btn text-primary" @click="router.push('/danh-muc/khach-hang/edit/' + kh.ma_khach_hang)" title="Sửa"><Pen size="16" /></button>
                <button class="action-btn text-danger" @click="handleDelete(kh.ma_khach_hang)" title="Xóa"><Trash size="16" /></button>
              </div>
